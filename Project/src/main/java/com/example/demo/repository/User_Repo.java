...
public interface User_Repo extends JpaRepository<User_Entity, Integer> 
{
	...
	public void userdeleteinfo(@Param("n") int userId);
	
	@Modifying
	@Transactional
	@Query(value="update user set userId=:n where userId=:nn",nativeQuery=true)
	public void userupdateinfo(@Param("n") int newid,@Param("nn") int oldid);
}
