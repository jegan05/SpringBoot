...
public interface Hotal_Repo extends JpaRepository<Hotal_Entity, Integer>
{
	...
	@Modifying
	@Transactional
	@Query(value="delete from hotal where hotalId=:n",nativeQuery=true)
	public String hotaldeleteinfo(@Param("n") int hotalId);
	
	@Modifying
	@Transactional
	@Query(value="update from hotal set hotalId=:n where hotalId=:nn",nativeQuery=true)
	public String hotalupdateinfo(@Param("n") int newid,@Param("nn") int oldid);
}
